...
**截图已准备（pic/截图/，全部 1280×800）：**

1. **主界面.png** — 打开扩展，展示内容预览 + 页面选择 + 保存按钮 + 预设标签
2. **公众号文章保存结果.png** — 展示公众号文章提取效果（标题、作者、富文本完整）
3. **右键菜单保存.png** — 在网页上右键，展示"保存到 Notion"菜单项
4. **设置面板.png** — 展示多空间管理 + 快捷键配置 + 保存预设管理
5. **保存成功提示.png** — 页面右上角绿色 toast，显示"✓ 已同步到 Notion"

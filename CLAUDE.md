...
- **当前版本**：v0.5.1（2026-05-24）
- **Phase 状态**：0-5 全部完成，v0.5.1 自动字段匹配已上线，详见 ROADMAP.md
...
1. **图片模式**：Base64 已移除（Notion API 不支持），v0.3.1 计划通过 File Upload API 实现真正的图片上传
2. **Token 刷新失败通知**：当前刷新失败静默，需弹窗提示用户
3. **CLAUDE.md 功能描述**：部分已实现功能描述滞后于代码，以实际代码为准
4. **自动化测试**：tests/verify.js 需更新到 v0.3 覆盖新特性
